...
#Creation of request url depending on the date.
def create_url(date):
...
async def get_exchange(n=None, lst=None):
    ...
    # Creating a list of urls for the last n days.
...
if __name__ == "__main__":
    ...
    try:
        n = int(sys.argv[1])
        if len(sys.argv) > 2:
            # If additional currencies are entered, we add them to the list of default currencies.
            lst_curr += sys.argv[2:]
    except IndexError:
        n = 1
